...
const SignUpPage = ({ setIsAuthenticated }: SignUpPageProps) => {
  ...
  const handleFocus = (field: 'fullname' | 'email' | 'password') => {
    const label = field === 'fullname' 
      ? fullnameRef.current?.parentElement?.querySelector('.floating-label')
      : field === 'email'
  ...
  useEffect(() => {
    if (fullnameRef.current?.value) {
      fullnameRef.current.parentElement?.querySelector('.floating-label')?.classList.add('text-[#d4af37]', 'font-semibold');
    }
  ...
  return (
    <div className="flex justify-center items-center min-h-screen bg-gradient-to-br from-[#f5f3ee] to-white p-8 relative overflow-hidden">
      ...
      <div className="w-full max-w-[450px] p-12 bg-white rounded-2xl shadow-lg text-center relative z-10 animate-fadeInUp border border-white/30">
        ...
        {/* Consent text */}
        <p className="text-gray-500 text-xs my-6 leading-relaxed">
          By signing up, you agree to our{' '}
          <a href="#" className="text-[#d4af37] font-medium hover:text-[#003366] hover:underline transition-colors duration-300">
            Terms of Service
          </a>{' '}
          and acknowledge our{' '}
          <a href="#" className="text-[#d4af37] font-medium hover:text-[#003366] hover:underline transition-colors duration-300">
            Privacy Policy
          </a>. We may send you important service notifications via email or SMS.
        </p>
